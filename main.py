...
# Set Google credentials (make sure your secret is correctly mounted on Render)
...
def whatsapp_reply():
    try:
        ...
        if num_media > 0:
            ...
            logging.info(f"Saved {filename} with size: {os.path.getsize(filename)} bytes")
            ...
            try:
                ...
                logging.error(f"OCR error: {ocr_err}")
                reply.body("Error processing invoice image.")
...
# Only needed if you run locally, not on Render
